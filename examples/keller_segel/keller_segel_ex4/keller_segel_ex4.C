...
const int exp_initial_cond=0; // Aply simple (exponential) boundary condition
...
Number initial_value_u(const Point& p,
			...
			const std::string&)
{
  Number x=p(0), y=p(1), z=p(2);
  const Number C0_u = global_param_C0_u;
  // return C0_u*exp(-x*x-y*y-z*z)*(4-x*x-y*y-z*z);
  Number ret = exp_initial_cond ?
    C0_u*exp(uSlopeFactor*(-x*x-y*y-z*z)) :
    C0_u*(1+tanh(uSlopeFactor*(1-x*x-y*y-z*z))); // C0_u*(1-x*x-y*y-z*z>0);
  return ret;
}
			...
			const std::string&)
{
  ...
  //  return C0_v*exp(-x*x-y*y-z*z)*(4-x*x-y*y-z*z);
  Number ret = exp_initial_cond ?
    C0_v*exp(uSlopeFactor*(-x*x-y*y-z*z)) :
    C0_v*(1+tanh(uSlopeFactor*(1-x*x-y*y-z*z))); // C0_v*(1-x*x-y*y-z*z>0);
  return ret;
}
...

{
  ...
  std::string input_file = options("infile", std::string("keller_segel_ex4.in"));
  ...
  const unsigned int n_mesh_refinement = options("nr", 2);
  ...
  // with it.
  //
  // Create a mesh object, with fixed dimension to be overridden later,
  // distributed across the default MPI communicator.
  Mesh mesh(init.comm(), 3); // dim=3

  // MeshTools::Generation::build_cube (mesh,
  std::cout << "Bulding sphere mesh with nb refinement="
	    << n_mesh_refinement << std::endl;
  ...
  global_param_C0_u = C0_u; // We save it as global variable because could't find another way :(
  ...
  std::string exodus_filename = "keller_segel_ex4.e";
