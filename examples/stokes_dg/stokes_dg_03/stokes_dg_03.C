...
// <h1>RRGalvan Stokes DG 02 - Interior Penalty Discontinuous Galerkin
// for Stokes Equations</h1> This example shows how Stokes
...
void matrix_reposition(
		        // Submatrices
		        DenseSubMatrix<Number>& Kuu, DenseSubMatrix<Number>& Kuv, DenseSubMatrix<Number>& Kup,
			DenseSubMatrix<Number>& Kvu, DenseSubMatrix<Number>& Kvv, DenseSubMatrix<Number>& Kvp,
			DenseSubMatrix<Number>& Kpu, DenseSubMatrix<Number>& Kpv, DenseSubMatrix<Number>& Kpp,
			// Rows and columns of diagonal submatrices
			int n_rows_u, int n_cols_u,
			int n_rows_v, int n_cols_v,
			int n_rows_p, int n_cols_p,
			// Indices for variables
			int u_var, int v_var, int p_var)
{
  int u_first_row = u_var*n_rows_u, u_first_col = u_var*n_cols_u;
  int v_first_row = v_var*n_rows_u, v_first_col = v_var*n_cols_u;
  int p_first_row = p_var*n_rows_u, p_first_col = p_var*n_cols_u; // Warning, asuming n_cols_u==n_cols_v !!!

  Kuu.reposition (u_first_row, u_first_col, n_rows_u, n_cols_u);
  Kuv.reposition (u_first_row, v_first_col, n_rows_u, n_cols_v);
  Kup.reposition (u_first_row, p_first_col, n_rows_u, n_cols_p);

  Kvu.reposition (v_first_row, u_first_col, n_rows_v, n_cols_u);
  Kvv.reposition (v_first_row, v_first_col, n_rows_v, n_cols_v);
  Kvp.reposition (v_first_row, p_first_col, n_rows_v, n_cols_p);

  Kpu.reposition (p_first_row, u_first_col, n_rows_p, n_cols_u);
  Kpv.reposition (p_first_row, v_first_col, n_rows_p, n_cols_v);
                         ...
                         const std::string & libmesh_dbg_var(system_name))
{
  ...
  const unsigned int p_var = stokesdg_system.variable_number ("p");

  // Get the Finite Element type for "u".  Note this will be
  // the same as the type for "v".
  FEType fe_u_type = stokesdg_system.variable_type(u_var);
  // Get the Finite Element type for "p".
  FEType fe_p_type = stokesdg_system.variable_type(p_var);
  ...
  const unsigned int pressure_jumps = es.parameters.get<unsigned int> ("pressure_jumps");
  ...
  const std::vector<std::vector<Real>> & psi_neighbor_face = fe_p_neighbor_face->get_phi();
  ...
  DenseSubMatrix<Number>
    Ke_uu(Ke), Ke_uv(Ke), Ke_up(Ke),
    Ke_vu(Ke), Ke_vv(Ke), Ke_vp(Ke),
    Ke_pu(Ke), Ke_pv(Ke), Ke_pp(Ke);
  ...
  DenseSubMatrix<Number>
    Kn_uu(Kn), Kn_uv(Kn), Kn_up(Kn),
    Kn_vu(Kn), Kn_vv(Kn), Kn_vp(Kn),
    Kn_pu(Kn), Kn_pv(Kn), Kn_pp(Kn);
  DenseSubMatrix<Number>
    Ken_uu(Ken), Ken_uv(Ken), Ken_up(Ken),
    Ken_vu(Ken), Ken_vv(Ken), Ken_vp(Ken),
    Ken_pu(Ken), Ken_pv(Ken), Ken_pp(Ken);
  DenseSubMatrix<Number>
    Kne_uu(Kne), Kne_uv(Kne), Kne_up(Kne),
    Kne_vu(Kne), Kne_vv(Kne), Kne_vp(Kne),
    Kne_pu(Kne), Kne_pv(Kne), Kne_pp(Kne);
	  ...
	  if (elem->neighbor_ptr(side) == libmesh_nullptr)
	    {
		  ...
		  for (unsigned int i=0; i<n_dofs_u; i++)
                    {
                      ...
                      for (unsigned int j=0; j<n_dofs_p; j++) // Warning: assuming n_dofs_u==n_dofs_v !!
			if(1) // WARNING, DELETE IT!!
                        {
		      	  // Velocity x pressure
		      	  Ke_up(i,j) += JxW_face[qp] * ( phi_face[i][qp]*qface_normals[qp](0)*psi_face[j][qp] );
		      	  Ke_vp(i,j) += JxW_face[qp] * ( phi_face[i][qp]*qface_normals[qp](1)*psi_face[j][qp] );
		      	}
		  ...
		  for (unsigned int i=0; i<n_dofs_p; i++)
		    {
                      ...
                      for (unsigned int j=0; j<n_dofs_u; j++)
                        {
		  	  // Velocity x pressure
		  	  Ke_pu(i,j) += JxW_face[qp] * ( phi_face[j][qp]*qface_normals[qp](0)*psi_face[i][qp] );
		  	  Ke_pv(i,j) += JxW_face[qp] * ( phi_face[j][qp]*qface_normals[qp](1)*psi_face[i][qp] );
		  	}
		  ...
		  libmesh_assert(refinement_type != "p"); // Not yet implemented in current version!!!
		  ...
		  std::vector<dof_id_type> neighbor_dof_indices_p;
		  ...
		  matrix_reposition(Kn_uu, Kn_uv, Kn_up,
				    Kn_vu, Kn_vv, Kn_vp,
				    Kn_pu, Kn_pv, Kn_pp,
				    n_neighbor_dofs_u, n_neighbor_dofs_u,
				    n_neighbor_dofs_v, n_neighbor_dofs_v,
				    n_neighbor_dofs_p, n_neighbor_dofs_p,
				    u_var, v_var, p_var);
		  matrix_reposition(Ken_uu, Ken_uv, Ken_up,
				    Ken_vu, Ken_vv, Ken_vp,
				    Ken_pu, Ken_pv, Ken_pp,
				    n_dofs_u, n_neighbor_dofs_u,
				    n_dofs_v, n_neighbor_dofs_v,
				    n_dofs_p, n_neighbor_dofs_p,
				    u_var, v_var, p_var);
		  matrix_reposition(Kne_uu, Kne_uv, Kne_up,
				    Kne_vu, Kne_vv, Kne_vp,
				    Kne_pu, Kne_pv, Kne_pp,
				    n_neighbor_dofs_u, n_dofs_u,
				    n_neighbor_dofs_v, n_dofs_v,
				    n_neighbor_dofs_p, n_dofs_p,
				    u_var, v_var, p_var);
		  ...
		  //
		  // WARNING: In this version we suppose, for u and v,
		  // the same FE and the same #dofs in all
		  // elements. ¡¡¡ This invalidates p-adaptivity !!!
		  //
                  for (unsigned int qp=0; qp<qface.n_points(); qp++)
                    {
                      // Ke Matrix. Integrate the element test function i
...
int main (int argc, char** argv)
{
  ...
  Order u_order                                = static_cast<Order>(input_file("u_order", 2));
  ...
  const unsigned int pressure_jumps            = input_file("pressure_jumps", 0);
  const bool singularity                       = input_file("singularity", true);
  const unsigned int dim                       = input_file("dimension", 3);
  const unsigned int nx                        = input_file("nx", 15);
  const unsigned int ny                        = input_file("ny", 15);
  ...
  equation_system.parameters.set<unsigned int>("pressure_jumps") = pressure_jumps;
  ...
  if (on_command_line("element_type"))
    {
      ...
      stokesdg_system.add_variable ("u", u_order, Utility::string_to_enum<FEFamily>(fe_str));
      stokesdg_system.add_variable ("v", u_order, Utility::string_to_enum<FEFamily>(fe_str));
      stokesdg_system.add_variable ("p", p_order, Utility::string_to_enum<FEFamily>(fe_str));
    }
  else {
    stokesdg_system.add_variable ("u", u_order, MONOMIAL);
    stokesdg_system.add_variable ("v", u_order, MONOMIAL);
    stokesdg_system.add_variable ("p", p_order, MONOMIAL);
    }
